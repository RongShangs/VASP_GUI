...
<img width="1673" height="983" alt="image" src="https://github.com/user-attachments/assets/5c131b06-a473-4e59-a36c-8fb91b675d6e" />


```

 ┌──────────────┬─────────────────────────────┬────────────────────┐
 │  左侧 280px   │         中间 弹性宽度         │     右侧 360px      │
 │              │                             │                    │
 │  服务器状态   │  面包屑导航 + 文件浏览器       │  [空闲] 95种计算卡片 │
 │  节点负载    │  [新建][上传][刷新]            │  🔍 搜索...        │
 │  VASP进程   │  ┌─────────────────────────┐ │  ▼ 基础计算         │
 │  连接状态    │  │ 名称      大小    时间    │ │  ⚛ SCF  🔧 优化   │
 │              │  │ INCAR    2.1K  02:30   │ │  ▼ 电子结构         │
 │ ──────────── │  │ KPOINTS  0.3K  02:30   │ │  📊 DOS  〰 能带   │
 │  Mini终端    │  │ POSCAR   5.2K  02:31   │ │  ...更多分类...     │
 │ [user@mgr]$  │  │ POTCAR   3.1K  02:31   │ │                    │
 │ ls -la       │  │ OUTCAR   1.2M  03:15   │ │  [运行] 实时监控     │
 │              │  └─────────────────────────┘ │  ┌────────────────┐ │
 │              │                             │  │ 控制台实时输出   │ │
 │              │  ▼ [INCAR] [KPOINTS] 标签页  │  │ vasp_std run... │ │
 │              │  ┌─────────────────────────┐ │  ├────────────────┤ │
 │              │  │ 可视化表单 | Monaco源码  │ │  │ 📊 能量收敛曲线  │ │
 │              │  │ ENCUT 400  ISIF 2       │ │  │  ╱‾‾╲          │ │
 │              │  │ NSW 60  IBRION 2        │ │  ├────────────────┤ │
 │              │  └─────────────────────────┘ │  │ 离子步 ██░ 6/10 │ │
 │              │                             │  │ 电子步 ███ 45/60│ │
...
**左侧面板** — 服务器状态、节点负载、VASP 进程信息、嵌入式 Web 终端，可直接执行集群命令

**中间面板** — 远程文件浏览器（右键菜单操作）、多标签页编辑器（可视化表单 ↔ Monaco 源码双态切换）、POSCAR 一键 3D 预览

**右侧面板** — 空闲时显示 95 种计算模板卡片，点击提交后切换为实时监控：控制台输出 + 能量收敛折线图 + 离子步/电子步双层进度条
