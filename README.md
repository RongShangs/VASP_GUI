...
- 服务器连接状态、节点 CPU/内存负载
- VASP 进程状态展示
- 嵌入式 Web 终端 (xterm.js)，可直接在集群上执行命令

**中间面板 (弹性宽度)**
- 远程文件浏览器，支持右键菜单（复制/粘贴/重命名/删除/下载/上传）
- 多标签页编辑器：双击文件进入可视化查看器，点击"编辑源码"切换到 Monaco 编辑器
- INCAR 可视化表单：双击参数值即可编辑，bool/enum/int/float/list 各类型智能匹配控件
- POSCAR 查看器：晶格矢量 + 原子坐标表格，一键切换到 3D 晶体结构预览
- 其他查看器：KPOINTS/POTCAR/OUTCAR/OSZICAR 各自有专用解析面板

**右侧面板 (360px)**
- 空闲态：15 大类 95 种 VASP 计算类型卡片，支持搜索筛选
- 点击卡片弹出参数确认框（自动读取项目 INCAR 预填），确认后一键提交到终端执行
- 运行态：实时监控面板，包含离子步/电子步双层进度条、能量收敛 ECharts 折线图、控制台实时输出
- 后处理页面：DOS 态密度 / 能带结构 / 能量收敛 / 光学性质四个标签页，下拉选择已完成任务即可加载交互式图表
